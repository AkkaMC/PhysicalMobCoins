...
public class MainInventory extends JInventory {
    ...
    public void open(Player player, Object... args) {
        ...
        addButton(34, new ItemStackBuilder(SkullUtils.getSkullFromUrl(Skin.REDSTONE_LAMP.getUrl())).displayName("&9&lCONFIGURE SETTINGS").lore("&7Click to manage the settings.").build())
                .setClick(e -> {
                    new SettingsInventory().open(player);
                });
        if (page > 1)
